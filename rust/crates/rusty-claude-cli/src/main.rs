...
impl LiveCli {
    ...
    /// Enable think-mode for this live session. The runtime's system prompt is
    /// already augmented when this is called, so callers should set this flag
    /// before invoking `prepare_turn_runtime` for the first time.
    fn set_think_mode(&mut self, enabled: bool) {
...
fn build_runtime_plugin_state(
    ...
    let loader = ConfigLoader::default_for(&cwd);
    let runtime_config = loader.load()?;
    build_runtime_plugin_state_with_loader(&cwd, &loader, &runtime_config, think_mode)
}
...
fn build_runtime_plugin_state_with_loader(
    ...
    let mut feature_config = runtime_config
        .feature_config()
        .clone()
        .with_hooks(runtime_config.hooks().merged(&plugin_hook_config));
    if think_mode {
        feature_config = feature_config.with_think_mode(true);
    }
...
fn build_runtime(
    ...
    let runtime_plugin_state = build_runtime_plugin_state(think_mode)?;
