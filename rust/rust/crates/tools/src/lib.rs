...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "OpenHands",
            description: "Execute bash or python code in an OpenHands-compatible sandbox. The endpoint is configurable via OPENHANDS_ENDPOINT env var or `--openhands-endpoint` flag (default http://localhost:8000). Returns the raw response body as a string.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "code": { "type": "string" },
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "OpenHands" => {
            let openhands_input: openhands::OpenHandsInput = from_value(input)?;
            maybe_enforce_permission_check_with_mode(
                enforcer,
                name,
                input,
