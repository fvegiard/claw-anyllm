...
pub struct ConversationRuntime<C, T> {
    ...
    /// When true, the runtime prepends the think-mode directive to the system
    /// prompt and parses assistant replies through
    /// [`crate::think_mode::extract_thinking`].
    think_mode: bool,
}
    ...
    T: ToolExecutor,
{
    ...
    pub fn run_turn(
        ...
        loop {
            ...
            if self.think_mode {
                if let Some(extraction) = self.apply_think_mode_extraction(&assistant_message) {
                    assistant_message = extraction;
                }
            }
    ...
    /// When think-mode is enabled, route the assistant text blocks through
    /// [`crate::think_mode::extract_thinking`] and return a new
    /// [`ConversationMessage`] with the thinking extracted into a
    /// [`ContentBlock::Thinking`] block (preserving any thinking already
    /// emitted via [`AssistantEvent::Thinking`]) and the visible text replaced
    /// by the structured split.
    ///
    /// Returns `None` when there's nothing to extract (e.g. the message has no
    /// text blocks). Returns the original message unchanged when extraction
    /// errors are encountered so the transcript stays deterministic; the
    /// unparsed text is preserved verbatim.
    fn apply_think_mode_extraction(
        &self,
        message: &ConversationMessage,
    ) -> Option<ConversationMessage> {
        use crate::think_mode::{extract_thinking, ThinkingError};
        use std::fmt::Write as _;

        let mut new_blocks: Vec<ContentBlock> = Vec::with_capacity(message.blocks.len());
        let mut touched = false;

        for block in &message.blocks {
            match block {
                ContentBlock::Text { text } => {
                    match extract_thinking(text) {
                        Ok(extraction) => {
                            if extraction.thinking.is_empty() {
                                // Passthrough: no thinking was emitted. Keep
                                // the original block so downstream consumers
                                // that key off identical strings are stable.
                                ...
                                if !extraction.visible.is_empty() {
                        ...
                        Err(ThinkingError::MultipleBlocks) => {
                            // Don't silently drop the malformed text — surface
                            // the error inside the transcript so the user can
                            // see what went wrong, and preserve the original
                            // block.
                            eprintln!(
                                "think_mode: ignoring assistant text with multiple <thinking> blocks"
                            );
                            let mut error_message = String::new();
                            let _ = write!(
                                error_message,
                                "[think_mode error] multiple <thinking> blocks detected in one assistant reply; the original text is preserved below."
                            );
                            new_blocks.push(ContentBlock::Text {
                                text: error_message,
                            });
                        ...
                        Err(ThinkingError::UnclosedBlock) => {
                            eprintln!(
                                "think_mode: ignoring assistant text with unclosed <thinking> block"
                            );
                            let mut error_message = String::new();
                            let _ = write!(
                                error_message,
                                "[think_mode error] unclosed <thinking> block in assistant reply; the original text is preserved below."
                            );
                            new_blocks.push(ContentBlock::Text {
                                text: error_message,
                            });
