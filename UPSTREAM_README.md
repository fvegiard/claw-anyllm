...
- [`docs/plugin-manifest.md`](./docs/plugin-manifest.md) — plugin manifest v2 schema (added in this fork), including how to register a new LLM provider by writing one JSON file

## Fork additions (`fvegiard/claw-code`)

This fork extends the upstream Rust workspace with three new capabilities:

### Think mode (`--think` flag)

When the flag is set, the runtime prepends a step-by-step reasoning directive to the system prompt, and the response handler parses `<thinking>...</thinking>` blocks back out of the assistant's reply. Run with:

```bash
./target/debug/claw --think prompt "say hello"
```

Malformed thinking blocks (unclosed or multi-block) are surfaced as errors so the caller notices rather than silently swallowing them. Empty `<thinking></thinking>` blocks are stripped cleanly. See `rust/crates/runtime/src/think_mode.rs` and `think_mode::tests`.

### OpenHands tool

A built-in tool that delegates bash / python execution to an HTTP endpoint. The wire protocol it speaks (`POST {path}` with `{"code": ..., "language": ...}` in the body) is a small shell-exec convention — not OpenHands's full SDK contract — but it matches what OpenHands's `/execute` route accepts and works against any backend that exposes that same shape.

Configurable end-to-end via environment variables:

| Env var | Default |
|---------|---------|
| `OPENHANDS_ENDPOINT`     | `http://localhost:8000` |
| `OPENHANDS_PATH`         | `/execute`              |
| `OPENHANDS_API_KEY`      | (unset)                 |
| `OPENHANDS_AUTH_HEADER`  | `Authorization`         |
| `OPENHANDS_AUTH_PREFIX`  | `Bearer `               |

Two recipes:

- **OpenHands defaults** — `Authorization: Bearer <key>` against `/execute`
- **OpenHands Agent Server** — `X-Session-API-Key: <key>` against `/api/exec` (set `OPENHANDS_AUTH_HEADER=X-Session-API-Key`, `OPENHANDS_AUTH_PREFIX="`)

See `rust/crates/tools/src/openhands.rs` and `openhands::tests`.

### Plugin manifest v2

Extends the plugin manifest with optional `providers`, `tools`, and `hooks` arrays — so any LLM backend (Mavis, Ollama, a custom OpenAI-compatible API, ...) can be plugged in by writing one JSON file. No Rust change required. Backwards-compatible: v1-shaped manifests without those arrays still parse. See [`docs/plugin-manifest.md`](./docs/plugin-manifest.md) and `manifest_v2::tests`.

### Test status (this fork)

All 30 new unit tests pass (`manifest_v2`: 9, `openhands`: 12, `think_mode`: 9). The full workspace builds with `cargo build --workspace` and emits zero warnings. Two pre-existing tests (`mcp_stdio::given_child_exits_after_discovery...`, `tests::powershell_runs_via_stub_shell`) fail in this sandbox because they need PowerShell on PATH and a real MCP child process — they are unrelated to this fork's changes.
